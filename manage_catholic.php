...
if (isset($_GET['approve'])) {
    ...
    header("Location: manage_catholic.php?success=approved");
    exit();
}
...
if (isset($_GET['reject'])) {
    ...
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$id, $denomination_filter]);
    header("Location: manage_catholic.php?success=rejected");
    exit();
}
...
if (isset($_GET['delete'])) {
    ...
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$id, $denomination_filter]);
    header("Location: manage_catholic.php?success=deleted");
    exit();
}
                    ...
                    <tr class="hover:bg-gray-50/30 transition-all group">
                        ...
                        <td class="px-8 py-6">
                            <div class="flex justify-end gap-2">
                                ...
                                <?php endif; ?>
                                <a href="?delete=<?php echo $candidate['id']; ?>" onclick="return confirm('Permanently delete this Catholic profile?')" class="p-3 bg-red-50 text-red-600 hover:bg-red-600 hover:text-white rounded-2xl transition-all shadow-sm" title="Delete">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
                <div>
                    <div class="flex items-center gap-3 mb-2">
                        <span class="px-3 py-1 bg-blue-100 text-blue-700 text-[10px] font-black uppercase tracking-widest rounded-full">Catholic Community</span>
                        <h1 class="text-4xl font-black text-gray-900 tracking-tighter">Catholic Registrations</h1>
                    </div>
                    <p class="text-gray-500 font-medium">Manage and review all catholic marriage candidate applications.</p>
                </div>
                
                <!-- Search Bar -->
                <div class="flex items-center gap-4">
                    <form action="" method="GET" class="relative group">
                        <input type="text" name="search" value="<?php echo htmlspecialchars($search); ?>" placeholder="Search Catholic names..." 
                ...
                <section>
                    <div class="flex items-center justify-between mb-8 px-2">
                        <h2 class="text-xl font-black text-gray-900 uppercase tracking-tighter flex items-center gap-3">
                            ...
                            Pending Catholic Approvals
                        </h2>
                        <span class="text-[10px] font-black bg-blue-500 text-white px-3 py-1 rounded-full"><?php echo count($pending); ?> WAITLIST</span>
                    </div>
