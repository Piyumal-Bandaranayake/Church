...
if (isset($_GET['approve'])) {
    ...
    header("Location: divorced_christian.php?success=approved");
    exit();
}
...
if (isset($_GET['reject'])) {
    ...
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$id, $denomination_filter]);
    header("Location: divorced_christian.php?success=rejected");
    exit();
}
...
if (isset($_GET['delete'])) {
    ...
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$id, $denomination_filter]);
    header("Location: divorced_christian.php?success=deleted");
    exit();
}
...
function renderTable($list) {
    if (empty($list)) {
        ...
        echo '<p class="text-gray-400 font-black uppercase tracking-widest text-[10px]">No divorced Christian applications found</p>';
        echo '</div>';
        return;
    }
    ?>
...
<?php 
$active_page = 'divorced_christian';
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
                <div>
                    <div class="flex items-center gap-3 mb-2">
                        <span class="px-3 py-1 bg-red-100 text-red-700 text-[10px] font-black uppercase tracking-widest rounded-full">Divorced Section</span>
                        <h1 class="text-4xl font-black text-gray-900 tracking-tighter">Christian Divorced</h1>
                    </div>
                    <p class="text-gray-500 font-medium italic">Review applications from divorced Christian members.</p>
                </div>
            </div>
                    ...
                    <div id="advancedFilters" class="<?php echo $active_filters > 0 ? '' : 'hidden'; ?> bg-white p-8 rounded-[2.5rem] shadow-2xl border border-gray-100 animate-slide-down">
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                            ...
                            <div class="flex items-end gap-2 lg:col-span-1">
                                ...
                                <a href="divorced_christian.php" class="p-3 bg-red-50 text-red-500 rounded-xl hover:bg-red-500 hover:text-white transition-all group" title="Clear Filters">
                ...
                <section>
                    <div class="flex items-center justify-between mb-8 px-2">
                        <h2 class="text-xl font-black text-gray-900 uppercase tracking-tighter flex items-center gap-3">
                            ...
                            Divorced Christian Waitlist
                        </h2>
                        <span class="px-4 py-1.5 bg-red-500 text-white rounded-full text-[10px] font-black shadow-lg shadow-red-200"><?php echo count($pending); ?> MATCHES</span>
                    </div>
