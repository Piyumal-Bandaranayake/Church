...
if (isset($_GET['approve'])) {
    ...
    header("Location: pending_catholic.php?success=approved");
    exit();
}
...
if (isset($_GET['reject'])) {
    ...
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$id, $denomination_filter]);
    header("Location: pending_catholic.php?success=rejected");
    exit();
}
...
if (isset($_GET['delete'])) {
    ...
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$id, $denomination_filter]);
    header("Location: pending_catholic.php?success=deleted");
    exit();
}
...
function renderTable($list) {
    if (empty($list)) {
        ...
        echo '<p class="text-gray-400 font-black uppercase tracking-widest text-[10px]">No pending Catholic applications found</p>';
        echo '</div>';
        return;
    }
    ?>
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
                <div>
                    <div class="flex items-center gap-3 mb-2">
                        <span class="px-3 py-1 bg-blue-100 text-blue-700 text-[10px] font-black uppercase tracking-widest rounded-full">Waitlist</span>
                        <h1 class="text-4xl font-black text-gray-900 tracking-tighter">Catholic Pending</h1>
                    </div>
                    <p class="text-gray-500 font-medium italic">Advanced filtering and review for Catholic applications.</p>
                </div>
            </div>
            ...
            <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 mb-12">
                <form action="" method="GET" class="space-y-8">
                    ...
                    <div class="flex justify-end gap-3 pt-4">
                        <a href="pending_catholic.php" class="px-6 py-3 bg-gray-100 text-gray-500 rounded-2xl text-xs font-bold uppercase tracking-widest hover:bg-gray-200 transition-all">Clear</a>
            ...
            <div class="space-y-16">
                <section>
                    <div class="flex items-center justify-between mb-8 px-2">
                        ...
                        <span class="px-4 py-1.5 bg-blue-500 text-white rounded-full text-[10px] font-black shadow-lg shadow-blue-200"><?php echo count($pending); ?> MATCHES</span>
                    </div>
